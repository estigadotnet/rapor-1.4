...
loadjs.ready("head", function() {
	...
	ft001_sekolahadd.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($t001_sekolah_add->KepalaSekolah->Required) { ?>
				elm = this.getElements("x" + infix + "_KepalaSekolah");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $t001_sekolah_add->KepalaSekolah->caption(), $t001_sekolah_add->KepalaSekolah->RequiredErrorMessage)) ?>");
			<?php } ?>
			<?php if ($t001_sekolah_add->NIPKepalaSekolah->Required) { ?>
				elm = this.getElements("x" + infix + "_NIPKepalaSekolah");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $t001_sekolah_add->NIPKepalaSekolah->caption(), $t001_sekolah_add->NIPKepalaSekolah->RequiredErrorMessage)) ?>");
			<?php } ?>
...
<?php } ?>
<?php if ($t001_sekolah_add->KepalaSekolah->Visible) { // KepalaSekolah ?>
	<div id="r_KepalaSekolah" class="form-group row">
		<label id="elh_t001_sekolah_KepalaSekolah" for="x_KepalaSekolah" class="<?php echo $t001_sekolah_add->LeftColumnClass ?>"><?php echo $t001_sekolah_add->KepalaSekolah->caption() ?><?php echo $t001_sekolah_add->KepalaSekolah->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $t001_sekolah_add->RightColumnClass ?>"><div <?php echo $t001_sekolah_add->KepalaSekolah->cellAttributes() ?>>
<span id="el_t001_sekolah_KepalaSekolah">
<input type="text" data-table="t001_sekolah" data-field="x_KepalaSekolah" name="x_KepalaSekolah" id="x_KepalaSekolah" size="30" maxlength="50" placeholder="<?php echo HtmlEncode($t001_sekolah_add->KepalaSekolah->getPlaceHolder()) ?>" value="<?php echo $t001_sekolah_add->KepalaSekolah->EditValue ?>"<?php echo $t001_sekolah_add->KepalaSekolah->editAttributes() ?>>
</span>
<?php echo $t001_sekolah_add->KepalaSekolah->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($t001_sekolah_add->NIPKepalaSekolah->Visible) { // NIPKepalaSekolah ?>
	<div id="r_NIPKepalaSekolah" class="form-group row">
		<label id="elh_t001_sekolah_NIPKepalaSekolah" for="x_NIPKepalaSekolah" class="<?php echo $t001_sekolah_add->LeftColumnClass ?>"><?php echo $t001_sekolah_add->NIPKepalaSekolah->caption() ?><?php echo $t001_sekolah_add->NIPKepalaSekolah->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $t001_sekolah_add->RightColumnClass ?>"><div <?php echo $t001_sekolah_add->NIPKepalaSekolah->cellAttributes() ?>>
<span id="el_t001_sekolah_NIPKepalaSekolah">
<input type="text" data-table="t001_sekolah" data-field="x_NIPKepalaSekolah" name="x_NIPKepalaSekolah" id="x_NIPKepalaSekolah" size="30" maxlength="50" placeholder="<?php echo HtmlEncode($t001_sekolah_add->NIPKepalaSekolah->getPlaceHolder()) ?>" value="<?php echo $t001_sekolah_add->NIPKepalaSekolah->EditValue ?>"<?php echo $t001_sekolah_add->NIPKepalaSekolah->editAttributes() ?>>
</span>
<?php echo $t001_sekolah_add->NIPKepalaSekolah->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
